...
class GeminiService:
    ...
    def _is_transient_provider_error(self, msg: str) -> bool:
        if not msg:
            return False
        m = msg.lower()
        return (
            "unavailable" in m
            or "high demand" in m
            or "temporarily" in m and "unavailable" in m
            or "503" in m
    ...
    async def generate_response(self, prompt: str, context: Dict[str, Any] = None) -> str:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if self._is_transient_provider_error(msg):
                    self.logger.warning(f"Gemini transient error (attempt {attempt + 1}/{max_attempts}): {e}")
                    if attempt < max_attempts - 1:
                        await asyncio.sleep(0.6 * (2 ** attempt))
                        continue
                    return "The AI model is temporarily busy right now. Please try again in a few seconds."

                self.logger.error(f"Error generating response: {e}")
                return "I couldn't generate a response right now due to a server error. Please try again."
    ...
    async def generate_streaming_response(self, prompt: str, context: Dict[str, Any] = None) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            if "quota exceeded" in msg.lower() or "free_tier_requests" in msg.lower():
                ...
                return
            if self._is_transient_provider_error(msg):
                self.logger.warning(f"Gemini transient error (stream): {e}")
                yield "The AI model is temporarily busy right now. Please try again in a few seconds."
                return
            self.logger.error(f"Error generating streaming response: {e}")
            yield "I couldn't generate a response right now due to a server error. Please try again."
