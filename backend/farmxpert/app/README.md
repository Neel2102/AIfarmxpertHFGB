...
Note: Specialized agents have been reorganized under `farmxpert/agents/*` (e.g. `farmxpert.agents.operations.weather_watcher`, `farmxpert.agents.supply_chain.market_intelligence_agent`, etc.).
...
**Usage**:
```python
from farmxpert.agents.operations.weather_watcher.agent import WeatherWatcherAgent

weather_data = WeatherWatcherAgent.analyze_weather({"latitude": 23.0225, "longitude": 72.5714})
```
...
market_data = await agent.handle({"query": "mandi price", "context": {"crops": ["cotton"], "location": {"state": "Gujarat"}}})
```
