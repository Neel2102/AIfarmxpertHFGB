...
class WeatherService:
    ...
    def get_weather(latitude: float, longitude: float) -> Optional[WeatherSnapshot]:
        ...
        """
        if not OPENWEATHER_API_KEY and not WEATHERAPI_KEY:
            logger.error("Weather service is not configured: missing OPENWEATHER_API_KEY and WEATHERAPI_KEY")
            return None
        ...
        if not WEATHERAPI_KEY:
            logger.error("OpenWeather failed and WEATHERAPI_KEY is not set; cannot use fallback WeatherAPI")
            return None
    ...
    def _fetch_openweather(lat: float, lon: float) -> Optional[WeatherSnapshot]:
        if not OPENWEATHER_API_KEY:
            logger.error("OPENWEATHER_API_KEY is not set; skipping OpenWeather request")
            return None
        ...
        try:
            ...
            try:
                ...
                body = None
            logger.error(f"OpenWeather HTTP error: status={status} detail={e} body={body}")
            return None
        except Exception as e:
    ...
    def _fetch_weatherapi(lat: float, lon: float) -> Optional[WeatherSnapshot]:
        if not WEATHERAPI_KEY:
            logger.error("WEATHERAPI_KEY is not set; skipping WeatherAPI request")
            return None
        ...
        try:
            ...
            try:
                ...
                body = None
            logger.critical(f"WeatherAPI HTTP error: status={status} detail={e} body={body}")
            return None
        except Exception as e:
            logger.critical(f"WeatherAPI fallback failed: {e}")
            return None
    ...
    def _fetch_openweather_forecast(lat: float, lon: float, days: int) -> List[WeatherForecast]:
        if not OPENWEATHER_API_KEY:
            logger.error("OPENWEATHER_API_KEY is not set; skipping OpenWeather forecast request")
            return []
        ...
        try:
            ...
            try:
                ...
                body = None
            logger.error(f"OpenWeather forecast HTTP error: status={status} detail={e} body={body}")
            return []
        except Exception as e:
    ...
    def _fetch_weatherapi_forecast(lat: float, lon: float, days: int) -> List[WeatherForecast]:
        if not WEATHERAPI_KEY:
            logger.error("WEATHERAPI_KEY is not set; skipping WeatherAPI forecast request")
            return []
        ...
        try:
            ...
            try:
                ...
                body = None
            logger.error(f"WeatherAPI forecast HTTP error: status={status} detail={e} body={body}")
            return []
        except Exception as e:
